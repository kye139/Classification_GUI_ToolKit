...
void CPropertiesWnd::InitPropList()
{
	...
	CMFCPropertyGridProperty* pImagePropertyGroup = new CMFCPropertyGridProperty(_T("이미지 속성"));
	pProp = new CMFCPropertyGridProperty(_T("이름"), _T(""));
	pProp->Enable(FALSE);
	pImagePropertyGroup->AddSubItem(pProp);

	pProp = new CMFCPropertyGridProperty(_T("채널"), _T(""));
	pProp->Enable(FALSE);
	pImagePropertyGroup->AddSubItem(pProp);

	pSize = new CMFCPropertyGridProperty(_T("이미지 크기"), 0, TRUE);
	pSize->Enable(FALSE);

	pProp = new CMFCPropertyGridProperty(_T("너비"), _T(""));
	pProp->Enable(FALSE);
	pSize->AddSubItem(pProp);

	pProp = new CMFCPropertyGridProperty(_T("높이"), _T(""));
	pProp->Enable(FALSE);
	pSize->AddSubItem(pProp);

	pImagePropertyGroup->AddSubItem(pSize);

	m_wndPropList.AddProperty(pImagePropertyGroup);
}
